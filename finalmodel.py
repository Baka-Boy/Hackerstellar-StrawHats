...
pdfFile = PdfPages("Visualization.pdf")

# Bar chart of total spending by category
plt.figure(figsize=(10, 6))
fig = sns.barplot(x='category', y='amount', data=category_spending)
plt.title('Total Spending by Category')
plt.xlabel('Category')
plt.ylabel('Total Spending')
pdfFile.savefig(plt.gcf())
...
fig = sns.barplot(x=day_of_week_spending.index, y=day_of_week_spending.values, palette='Blues_d')
...
plt.figure(figsize=(8,6))
fig = sns.barplot(x=top_descriptions.index, y=top_descriptions.values, palette='Blues_d')
...
fig = sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', vmin=-1, vmax=1)
plt.title('Correlation Between Spending Categories')
pdfFile.savefig(plt.gcf())

# Pie chart of spending by category
plt.figure(figsize=(10, 6))
fig = plt.pie(category_spending['amount'], labels=category_spending['category'], autopct='%1.1f%%')
...
plt.figure(figsize=(10, 6))
fig = sns.lineplot(x='date', y='amount', data=daily_spending)
...
plt.figure(figsize=(10, 6))
fig = sns.scatterplot(x='date', y='category', hue='amount', size='amount', data=spending_data)
